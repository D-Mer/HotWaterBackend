...
def logistic_classify(x, y):
    ...
    accs = []
    kf = StratifiedKFold(n_splits=N_SPLIT, shuffle=True, random_state=None)
...
def svc_classify(x, y, search):
    ...
    for train_index, test_index in kf.split(x, y):
        ...
        if search:
            ...
            classifier = GridSearchCV(SVC(), params, cv=5, scoring='accuracy', verbose=0, n_jobs=12)
        else:
            classifier = SVC(C=10)
...
def randomforest_classify(x, y, search):
    ...
    for train_index, test_index in kf.split(x, y):
        ...
        if search:
            ...
            classifier = GridSearchCV(RandomForestClassifier(n_jobs=12), params, cv=5, scoring='accuracy', verbose=0, n_jobs=12)
        else:
            classifier = RandomForestClassifier(n_jobs=12)
        classifier.fit(x_train, y_train)
        accuracies.append(accuracy_score(y_test, classifier.predict(x_test)))
